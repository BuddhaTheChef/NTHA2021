...
class PuzzlePiece():
    ...
    def __str__(self):
        return f"ID: {self.id}, LENGTH: {self.l}, WIDTH: {self.w}, ROTATED: {self.rotate}"
...
def solve(board, remaining, used_pieces=[]):
    ...
    if remaining == 0:
        return 
    used_pieces.append(remaining)
    # Recursive call to call remaining pieces until reaching 0
    # print(used_pieces)
    solve(board, remaining-1, used_pieces)
    return used_pieces

#test solution for solve()  
# solve(56,12,used_pieces=[])
